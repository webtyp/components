...
// class (a bare part name like ".targetlist__sel-count"). The generator
...
func TestTargetList_MasterCheckHeaderReservesHeightAlways(t *testing.T) {
	css := (&TargetList{}).RenderCSS().String()
	header := classRules(css, ".targetlist__sel-header")
	if header == "" {
	...
	spacer := classRules(css, ".targetlist__sel-spacer")
	if spacer == "" {
		t.Fatal("expected a rule for .targetlist__sel-spacer")
	}
...
func TestTargetList_MasterCheckBoxHiddenUntilSelectionMode(t *testing.T) {
	css := (&TargetList{}).RenderCSS().String()
	body := classRules(css, ".targetlist__sel-all")
	if body == "" {
		t.Fatal("expected a rule for .targetlist__sel-all")
	}
	if !strings.Contains(body, "display: none") {
		t.Errorf("sel-all must be hidden by default, block:\n%s", body)
	}
	if !strings.Contains(css, `.targetlist[data-open="true"] .targetlist__sel-all {`) {
...
func TestTargetList_MasterCheckCountAlwaysVisible(t *testing.T) {
	css := (&TargetList{}).RenderCSS().String()
	body := classRules(css, ".targetlist__sel-count")
	if body == "" {
		t.Fatal("expected a rule for .targetlist__sel-count")
	}
	if strings.Contains(body, "display: none") {
...
// TestTargetList_MasterCheckUsesSharedGlyphs cannot catch: it only proves the
// reference exists, not that the symbol backing it does.
func TestTargetList_IconSvgRegistersSelectAll(t *testing.T) {
	out := (&TargetList{}).IconSvg().String()
