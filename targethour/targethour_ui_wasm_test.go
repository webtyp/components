...
// (the exact bug reported against the reservation demo).
func TestMasterCheckCountUpdatesAfterReload(t *testing.T) {
	...
	if got := query(t, ".targethour__sel-count").Get("textContent").String(); got != "0 / 2" {
		t.Fatalf("count = %q, want 0 / 2", got)
	}

	th.SetItems([]targethour.Item{{ID: "1"}, {ID: "2"}, {ID: "3"}})
	if got := query(t, ".targethour__sel-count").Get("textContent").String(); got != "0 / 3" {
